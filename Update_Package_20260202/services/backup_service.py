...
from services.transfer_service import file_lock
...
BACKUP_CONFIGS = DEFAULT_BACKUP_CONFIGS.copy()

# Status tracking
BACKUP_STATUS = {
    'products': {'last_run': None, 'status': 'pending', 'message': ''},
    'cashiers_open': {'last_run': None, 'status': 'pending', 'message': ''},
    'insumos': {'last_run': None, 'status': 'pending', 'message': ''},
    'tables': {'last_run': None, 'status': 'pending', 'message': ''},
    'reception': {'last_run': None, 'status': 'pending', 'message': ''},
    'logs': {'last_run': None, 'status': 'pending', 'message': ''},
    'full_system': {'last_run': None, 'status': 'pending', 'message': ''}
}
    ...
    def _perform_backup(self, job_name, config):
        ...
        try:
            if job_name == 'cashiers_open':
                src_path = os.path.join(DATA_DIR, 'cashier_sessions.json')
                open_sessions = []
            ...
            if 'source_dir' in config:
                ...
                if job_name == 'full_system':
                    # Check if drive exists (G:)
                    if not os.path.exists(os.path.splitdrive(dest_dir)[0]):
                ...
                backup_name = f"{job_name}_{timestamp}" # make_archive adds extension
