...
def test_finance_commission_refresh_scores_no_manual_role_gate_in_route_source():
    source = Path(__file__).resolve().parents[1] / "app" / "blueprints" / "finance" / "routes.py"
    text = source.read_text(encoding="utf-8")
    marker = "def finance_commission_refresh_scores(cycle_id):"
...
def test_finance_commission_refresh_scores_policy_has_action_scope_and_role():
    ...
    policy = by_endpoint.get("finance.finance_commission_refresh_scores")
    ...
    assert action.get("name_by_method", {}).get("POST") == "action.finance.finance_commission_refresh_scores.post"
    scope = policy.get("scope", {})
    assert "scope.finance.write" in (scope.get("scopes_any") or [])
    assert "scope.department" in (scope.get("scopes_all") or [])


def test_finance_commission_refresh_scores_policy_driven_access_matrix(monkeypatch):
    ...
    admin_resp = admin_client.post(
        "/finance/commission/cycle-x/refresh_scores",
