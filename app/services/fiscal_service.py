...
def get_nfce_status(nfe_id, settings):
    if not nfe_id:
        return {"success": False, "message": "ID da NFC-e ausente"}
    client_id = settings.get('client_id')
    client_secret = settings.get('client_secret')
    base_url = "https://api.sandbox.nuvemfiscal.com.br" if settings.get('environment') == 'homologation' else "https://api.nuvemfiscal.com.br"
    audience = base_url + "/"
    ...
    api_url = f"{base_url}/nfce/{nfe_id}"
    headers = {"Authorization": f"Bearer {token}"}
    try:
        ...
        if response.status_code != 200:
            try:
                ...
                msg = response.text
            return {"success": False, "message": msg or f"HTTP {response.status_code}"}
        data = response.json() or {}
        raw_status_candidates = [
            data.get("status"),
            data.get("situacao"),
            data.get("status_sefaz"),
            (data.get("autorizacao") or {}).get("status"),
            (data.get("autorizacao") or {}).get("situacao"),
            (data.get("sefaz") or {}).get("status"),
            (data.get("sefaz") or {}).get("situacao")
        ]
        status_norm = ""
        for val in raw_status_candidates:
            if isinstance(val, str) and val.strip():
                status_norm = val.lower().strip()
                break
        authorized = status_norm in ["autorizada", "autorizado", "aprovada", "authorized"]
        if not status_norm and isinstance(data.get("autorizado"), bool):
            authorized = bool(data.get("autorizado"))
            status_norm = "autorizada" if authorized else "nao_autorizada"
        message = ""
        if isinstance(data.get("autorizacao"), dict):
            message = data["autorizacao"].get("motivo_status") or ""
        if not message:
