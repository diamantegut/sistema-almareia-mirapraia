...
def _get_sales_flat():
    ...
    for order in data or []:
        ...
        for it in order.get('items', []):
            ...
            table_ref = order.get('table_id') or ''
            sale_context = f"Mesa {table_ref}" if table_ref else "Venda direta"
            items.append({
                'sale_id': close_id,
                'product': it.get('name'),
                ...
                'justification': sale_context
            })
            for acc in it.get('accompaniments', []) or []:
                ...
                items.append({
                    'sale_id': close_id,
                    'product': acc_name,
                    'product_id': str(acc_id) if acc_id is not None else '',
                    'qty': qty,
                    'total': round(qty * acc_price, 2),
                    'timestamp': ts,
                    'timestamp_str': closed_at or '',
                    'status': status,
                    'customer': customer,
                    'user': order.get('waiter') or order.get('staff_name') or '',
                    'justification': f"Vendido como acompanhamento de {it.get('name')} | {sale_context}"
                })
    open_orders = load_table_orders()
    if isinstance(open_orders, dict):
        for table_id, order in open_orders.items():
            if not isinstance(order, dict):
                continue
            if str(order.get('status', 'open')).lower() != 'open':
                continue
            customer = order.get('customer_name') or ''
            user_name = order.get('waiter') or order.get('staff_name') or ''
            for it in order.get('items', []) or []:
                ...
                for acc in it.get('accompaniments', []) or []:
                    if isinstance(acc, dict):
                        try:
                            acc_total += float(acc.get('price', 0) or 0)
                        except Exception:
                            pass
                total_val = qty * (base_price + comps_total + acc_total)
                launched_at = it.get('created_at') or order.get('opened_at') or ''
                try:
                    ts = datetime.strptime(launched_at, '%d/%m/%Y %H:%M') if launched_at else None
                except Exception:
                    ts = None
                sale_id = f"OPEN_{table_id}_{it.get('id') or ''}".strip('_')
                sale_context = f"Mesa {table_id} (lançamento)"
                items.append({
                    'sale_id': sale_id,
                    'product': it.get('name'),
                    'product_id': it.get('product_id'),
                ...
                for acc in it.get('accompaniments', []) or []:
                    ...
                    items.append({
                        ...
                        'customer': customer,
                        'user': user_name,
                        'justification': f"Lançado como acompanhamento de {it.get('name')} | {sale_context}"
                    })
...
def menu_sales_history_api():
    ...
    total_qty = sum(it['qty'] for it in filtered if str(it['status']).lower() in ['closed', 'open'])
    cancelled_qty = sum(1 for it in filtered if str(it['status']).lower() == 'cancelled')
    net_qty = total_qty - cancelled_qty
    total_value = round(sum(it['total'] for it in filtered if str(it['status']).lower() in ['closed', 'open']), 2)
    return jsonify({
        'success': True,
        'items': [
            {
                ...
                'status': 'concluída' if str(it['status']).lower() == 'closed' else ('lançada' if str(it['status']).lower() == 'open' else 'cancelada'),
...
def menu_sales_history_export():
    ...
    total_qty = sum(it['qty'] for it in items if str(it['status']).lower() in ['closed', 'open'])
    cancelled_qty = sum(1 for it in items if str(it['status']).lower() == 'cancelled')
    net_qty = total_qty - cancelled_qty
    total_value = round(sum(it['total'] for it in items if str(it['status']).lower() in ['closed', 'open']), 2)
    if fmt == 'pdf':
        output = io.BytesIO()
    ...
    for it in items:
        ...
        status_label = 'concluída' if str(it['status']).lower() == 'closed' else ('lançada' if str(it['status']).lower() == 'open' else 'cancelada')
        ws.write(row, 4, status_label)
