...
def restaurant_table_order(table_id):
            ...
            try:
                ...
                for item_data in batch_items:
                    ...
                    if item_data.get('accompaniments'):
                        accompaniment_menu_items = []
                        for acc_id in item_data['accompaniments']:
                            acc_prod = products_map.get(str(acc_id))
                            if not acc_prod:
                                acc_prod = next((p for p in menu_items if p.get('name') == str(acc_id)), None)
                            if acc_prod:
                                accompaniment_menu_items.append(acc_prod)
                                ...
                                order_item['accompaniments'].append({
                                    'id': None,
                                    'name': sanitize_input(str(acc_id)),
                                    'price': 0.0
                                })
                        for acc_prod in accompaniment_menu_items:
                            ...
                            if acc_prod.get('recipe'):
                                for ingred in acc_prod.get('recipe', []):
                                    ...
                                    with file_lock(STOCK_ENTRIES_FILE):
                                        save_stock_entry({
                                            'id': f"SALE_REST_ACC_{table_id}_{order_item['id']}_{ing_key}",
                                            'user': session.get('user', 'Sistema'),
                                            ...
                                            'qty': -total_needed,
                                            'price': insumo_data.get('price', 0),
                                            'invoice': f"Venda Restaurante: Acomp {acc_prod.get('name')} de {product.get('name')}",
                                            'date': datetime.now().strftime('%d/%m/%Y'),
                                            'entry_date': datetime.now().strftime('%d/%m/%Y %H:%M')
                                        })
                                    deducted_ok = True
                                ...
                                if acc_stock_product:
                                    log_stock_action(
                                        user=session.get('user'),
                                        action='saida',
                                        product=acc_stock_product['name'],
                                        qty=qty,
                                        details=f"Venda Mesa {table_id} | Acomp de {product.get('name')}",
                                        department='Restaurante'
                                    )
                                    with file_lock(STOCK_ENTRIES_FILE):
                                        save_stock_entry({
                                            'id': f"SALE_REST_ACC_{table_id}_{order_item['id']}_{str(acc_stock_product.get('id') or acc_stock_product.get('name'))}",
                                            'user': session.get('user', 'Sistema'),
                                            'product_id': acc_stock_product.get('id'),
                                            'product': acc_stock_product['name'],
                                            'supplier': f"VENDA: Mesa {table_id}",
                                            'qty': -qty,
                                            'price': acc_stock_product.get('price', 0),
                                            'invoice': f"Venda Restaurante: Acomp {acc_prod.get('name')} de {product.get('name')}",
                                            'date': datetime.now().strftime('%d/%m/%Y'),
                                            'entry_date': datetime.now().strftime('%d/%m/%Y %H:%M')
                                        })
            ...
            try:
                ...
                with file_lock(TABLE_ORDERS_FILE):
                    ...
                    for item in order.get('items', []):
                        ...
                        try:
                            ...
                            item_qty = 1.0
                        comps_price = sum(float(c.get('price', 0) or 0) for c in item.get('complements', []))
                        recalculated_total += item_qty * (item_price + comps_price)
                ...
                for item in order['items']:
                    ...
                    for component in expand_order_item_stock_components(item):
                        ...
                        if menu_item_match and menu_item_match.get('recipe'):
                            ...
                            for ingred in menu_item_match.get('recipe', []):
                                ...
                                stock_entries_to_add.append({
                                    'id': f"SALE_ACC_RECIPE_{item_uuid}_{str(component_pid or component_name)}_{str(ing_id)}",
                                    'date': datetime.now().strftime('%d/%m/%Y'),
                                    'product_id': insumo_data.get('id'),
                                    'product': insumo_data['name'],
                                    ...
                                    'invoice': f"Mesa {table_id} | Acomp de: {parent_name}",
                                    'user': session.get('user')
                                })
                            continue
                        ...
                        if not product_obj:
                            continue
                        deducted_products.append(product_obj)

                        origin_tag = f" | Acomp de {parent_name}" if origin == 'acompanhamento' else ""
                        log_stock_action(
                            user=session.get('user'),
                            action='saida',
                            product=product_obj['name'],
                            qty=qty,
                            details=f"Venda Mesa {table_id}{origin_tag}",
                            department='Restaurante'
                        )

                        stock_entries_to_add.append({
                            'id': f"SALE_{item_uuid}_{str(component_pid or component_name)}",
                            'date': datetime.now().strftime('%d/%m/%Y'),
                ...
                for p_obj in deducted_products:
                    unique_deducted[str(p_obj.get('id') or p_obj.get('name'))] = p_obj

                for p_obj in unique_deducted.values():
                    p_name = p_obj.get('name')
                    min_stock = float(p_obj.get('min_stock', 0))
                    if p_name and min_stock > 0:
                ...
                try:
                    ...
                    customer_doc = re.sub(r'\D', '', str(request.form.get('customer_cpf_cnpj', '') or ''))
                    if len(customer_doc) not in (11, 14):
                        customer_doc = ''
                    pool_entry_id = FiscalPoolService.add_to_pool(
                        origin='restaurant',
                ...
                if request.form.get('emit_invoice') == 'on' and pool_entry_id:
                    try:
                        ...
                        if emission_result.get('success', 0) > 0:
                            try:
                                emitted_entry = FiscalPoolService.get_entry(pool_entry_id) or {}
                                entry_items = emitted_entry.get('items') or []
                                invoice_data = {
                                    'valor_total': float(emitted_entry.get('total_amount', 0.0) or 0.0),
                                    'ambiente': 'homologacao',
                                    'items': []
                                }
                                for it in entry_items:
                                    qty = float(it.get('qty', 0.0) or 0.0)
                                    unit_price = float(it.get('price', 0.0) or 0.0)
                                    row_total = float(it.get('total', qty * unit_price) or 0.0)
                                    invoice_data['items'].append({
                                        'code': str(it.get('id') or it.get('product_id') or ''),
                                ...
                                ok_print, err_print = print_fiscal_receipt({}, invoice_data, force_print=True)
                                if ok_print:
                            ...
                            emitted_entry = FiscalPoolService.get_entry(pool_entry_id) or {}
                            emission_error = emitted_entry.get('last_error') or 'Falha ao emitir NFC-e agora.'
                            flash(f'Mesa fechada, mas emissão imediata falhou: {emission_error}')
                        ...
                        for component in expand_order_item_stock_components(item):
                            ...
                            if menu_item_match and menu_item_match.get('recipe'):
                                ...
                                for ingred in menu_item_match.get('recipe', []):
                                    ...
                                    with file_lock(STOCK_ENTRIES_FILE):
                                        save_stock_entry({
                                            'id': f"STAFF_ACC_RECIPE_{item_uuid}_{str(component_pid or component_name)}_{str(ing_id)}",
                                            'date': datetime.now().strftime('%d/%m/%Y'),
                                            'product_id': insumo_data.get('id'),
                                            'product': insumo_data['name'],
                                            'qty': -abs(total_needed),
                                            'unit': insumo_data.get('unit', 'un'),
                                            'price': insumo_data.get('price', 0),
                                            'supplier': 'Consumo Interno (Acompanhamento)',
                                            'invoice': f"Func {order.get('staff_name')} | Acomp de: {parent_name}",
                                            'user': session.get('user')
                                        })
                                continue
                            ...
                            log_stock_action(
                                ...
                                product=product_obj['name'],
                                qty=qty,
                                details=f"Consumo Funcionario {order.get('staff_name')}{details_suffix}",
                                department='Restaurante'
                            )
                            with file_lock(STOCK_ENTRIES_FILE):
                                save_stock_entry({
                                    'id': f"STAFF_{item_uuid}_{str(component_pid or component_name)}",
                                    'date': datetime.now().strftime('%d/%m/%Y'),
                                    'product_id': product_obj.get('id'),
                                    'product': product_obj['name'],
                                    'qty': -abs(qty),
                                    'unit': product_obj.get('unit', 'un'),
                                    'price': product_obj.get('price', 0),
                                    'supplier': 'Consumo Interno (Acompanhamento)' if origin == 'acompanhamento' else 'Consumo Interno',
                                    'invoice': f"Func {order.get('staff_name')} | Acomp de: {parent_name}" if origin == 'acompanhamento' else f"Func {order.get('staff_name')}",
                                    'user': session.get('user')
                                })
    ...
    if str_table_id in orders:
        ...
        for item in order.get('items', []):
            ...
            try:
                ...
                item_qty = 1.0
            comps_price = sum(float(c.get('price', 0) or 0) for c in item.get('complements', []))
            displayed_subtotal += item_qty * (item_price + comps_price)

        # Regra: Funcionário não paga taxa de serviço e recebe 20% de desconto
        if order.get('customer_type') == 'funcionario':
            staff_discount = round(displayed_subtotal * 0.20, 2)
            service_fee = 0.0
            grand_total = max(0.0, displayed_subtotal - staff_discount)
            ...
            service_fee = displayed_subtotal * 0.1
    ...
    opened_by_display = 'Não informado'
    current_order = orders.get(str_table_id)
    if isinstance(current_order, dict):
        opened_by_user = str(current_order.get('opened_by') or '').strip()
        if opened_by_user:
            if isinstance(users, dict) and opened_by_user in users and isinstance(users.get(opened_by_user), dict):
                opened_by_display = str(users[opened_by_user].get('full_name') or opened_by_user).strip() or opened_by_user
            else:
    ...
    try:
        ...
        if dest_table_id not in orders:
            ...
            orders[dest_table_id] = {
                ...
                'waiter': session.get('user'),
                'opened_by': (orders.get(source_table_id, {}) or {}).get('opened_by') or session.get('user'),
...
def public_waiting_list():
    ...
    position = 0
    status_view = None
    submission_token = session.get('waiting_list_submission_token')
    if not submission_token:
        submission_token = uuid.uuid4().hex
        session['waiting_list_submission_token'] = submission_token
    
    if user_entry_id:
        position = waiting_list_service.get_entry_position(user_entry_id)
        entry = waiting_list_service.get_customer_entry(user_entry_id)
        if not entry:
            session.pop('waiting_list_id', None)
        else:
            status_view = waiting_list_service.get_public_status_view(entry.get('status'))
            if status_view.get('is_final'):
                if status_view.get('code') in {'desistiu', 'cancelado_pela_equipe', 'nao_compareceu', 'expirado'}:
                    session.pop('waiting_list_id', None)
                    entry = None
                    position = 0
                    status_view = None
    ...
    if request.method == 'POST':
        ...
        if not token or token != session.get('waiting_list_submission_token'):
            flash('Sua solicitação já foi processada. Atualize a página para tentar novamente.')
            return redirect(url_for('restaurant.public_waiting_list'))
        session['waiting_list_submission_token'] = uuid.uuid4().hex
        ...
        country_dial_code = request.form.get('country_dial_code', '')
...
def cancel_waiting_list_entry(id):
    if session.get('waiting_list_id') == id or session.get('role') in ['admin', 'gerente', 'recepcao']:
        waiting_list_service.update_customer_status(
            id,
            'desistiu',
