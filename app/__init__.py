...
def create_app(config_name=None):
    ...
    def enforce_permissions():
        if app.config.get('EXTERNAL_OPEN_MODE'):
            return None
        try:
            from app.services.permission_service import enforce_request_access, run_shadow_authorization_probe
            pilot_response = run_shadow_authorization_probe()
            if pilot_response is not None:
                return pilot_response
            return enforce_request_access()
        except Exception as exc:
            if str(exc).startswith("AUTHZ_DEV_POLICY_MISSING"):
                raise
            try:
                from app.services.permission_service import handle_authorization_flow_exception
                hardened_response = handle_authorization_flow_exception(exc)
                if hardened_response is not None:
